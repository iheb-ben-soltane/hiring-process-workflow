...
public class AnnoncePosteHandler implements JobHandler {
    ...
    private static final String MESSAGE_NAME = "msg_posteAnnonce";

    @Override
    public void handle(JobClient client, ActivatedJob job) throws Exception {

        final Map<String, Object> inputVariables = job.getVariablesAsMap();
        final int postAnnonceId = 123456789;
        final String postName = (String) inputVariables.get("poste_demande");
        ...
        try (final ZeebeClient travelAgencyClient = ZeebeClient.newClientBuilder()
            ...
            System.out.println("this is it" + postAnnonceId);
            messageVariables.put("postAnnonceId", postAnnonceId);
            messageVariables.put("posteDemande", postName);


            //Send the message
            travelAgencyClient.newPublishMessageCommand()
                    .messageName(MESSAGE_NAME)
                    .correlationKey(String.valueOf(postAnnonceId))
                    .variables(messageVariables)
                    .send()
                    .join();

            System.out.println(postAnnonceId + " annonce bien reçue et voici le message envoyé " +postName);
